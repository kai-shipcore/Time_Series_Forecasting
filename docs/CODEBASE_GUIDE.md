...
| `src/ml/seasonal.py` | ~95 | The ML track's own seasonal factors, read from the `ML_*` holiday settings in `config.py`. Defaults equal the prototype's, so the two are interchangeable until deliberately diverged; `matches_prototype()` asserts that. Exists so an ML-side factor change cannot move the prototype, which is the accuracy bar. |
...
| `scripts/ml_12_seasonal_split_check.py` | ~70 | Verifies the ML/prototype factor split is behaviour-preserving: factors identical, and v-base and v3 reproducing their recorded figures on all three windows. Run after any change to either factor source. |
