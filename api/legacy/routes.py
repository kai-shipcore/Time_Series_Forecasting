...
WHAT IS NOT HERE
----------------
`/health`, `/forecast-status/{job_id}`, `/cancel-forecast/{job_id}` and
`/forecast-last-run` stayed in `api/main.py`. They look legacy by name but the
job registry behind them is shared: `/planning/run-forecast` on the ML track
returns a job_id polled through the same three endpoints, and the Action List
polls them. Moving them here would have broken the ML track's job polling.
...
from config import (
    FREQUENCY, USE_SEASONAL_ADJUSTMENT, OUTPUTS_REPORTS, TEST_WEEKS, CONFORMAL_LEVELS,
    FORECAST_HORIZON, SHORT_HISTORY_WEEKS, MIN_SIM_HISTORY_WEEKS, MAX_CONFORMAL_WINDOWS,
    MIN_CONFORMAL_WINDOWS,
)

HORIZON_WEEKS = round(FORECAST_HORIZON / 7)

from src.db import (
    read_latest_forecast, read_actuals, read_segments, get_engine, get_global_start,
    _product_type_where, create_job, append_job_lines, touch_job, set_job_pgid, finish_job,
    get_job, job_cancel_requested, request_job_cancel,
)
...
def _data_version() -> str:
    ...
    Reads the legacy fc_forward_forecasts table, which is why this and
    _cached_response live here rather than in api/common.py: the ML track has
    its own store and never calls either.
    """
    engine = get_engine()
...
@router.post("/segment-simulate-job/{segment}")
...
@router.get("/segment-simulate-result/{job_id}")
...
@router.post("/cancel-simulation/{job_id}")
...
@router.get("/segment-simulate/{segment}")
...
@router.get("/segment-detail/{segment}")
